...
class LocalProvider:
    ...
    def filter_moments(self, candidates: List[Dict], transcript: List[Dict]) -> List[Dict]:
        ...
        logger.info("Filtering with Local Smart Analysis...")
    ...
    def _check_clarity(self, moment: Dict) -> float:
        ...
        Returns 0-10 points based on word count.
        """
        text = moment.get("text", "").lower()
        word_count = len(text.split())

        if word_count < 5:
            return 2.0  # Too short, incomplete
        elif word_count > 150:
            return 5.0  # Long but may still be good
        else:
            return 10.0  # Just right

    def score_moments(self, moments: List[Dict], transcript: List[Dict]) -> List[Dict]:
        """Score moments using smart local analysis."""
        logger.info("Scoring with Local Smart Analysis...")
