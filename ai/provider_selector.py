...
from ai.base_provider import ProviderConfig
...
def get_available_providers(logger) -> Dict[str, Tuple]:
    ...
    if xai_key and xai_key.strip():
        try:
            from ai.xai_provider import XAIProvider
            provider = XAIProvider(ProviderConfig(name='xai', api_key=xai_key))
            try:
                if provider.health_check():
                    providers["xai"] = (lambda: XAIProvider(ProviderConfig(name='xai', api_key=xai_key)), "✓ xAI (Grok)", True)
                else:
                    providers["xai"] = (lambda: XAIProvider(ProviderConfig(name='xai', api_key=xai_key)), "✗ xAI (health check failed)", False)
            except Exception as e:
                providers["xai"] = (lambda: XAIProvider(ProviderConfig(name='xai', api_key=xai_key)), f"✗ xAI (Invalid key or connection error)", False)
        except ImportError:
            providers["xai"] = (None, "⚠ xAI (library not installed: pip install openai)", False)
    else:
        providers["xai"] = (None, "⚠ xAI (no API key set)", False)

    # Local provider is always available
    try:
        from ai.local_provider import LocalProvider
        providers["local"] = (LocalProvider, "✓ Local (offline)", True)
    except ImportError:
        providers["local"] = (None, "⚠ Local provider unavailable", False)

    return providers


def select_provider(provider_name=None, logger=None, safe_mode: bool = False):
    """
    ...
    Args:
        ...
        safe_mode: If True, prefer local provider and conservative processing
    ...
    if provider_name is None:
        ...
        if provider_name:
            logger.info(f"Using provider from environment: {provider_name}")
    ...
    # If anti-hallucination safe mode is enabled, prefer local provider
    if safe_mode and not provider_name:
        logger.info("Anti-hallucination mode enabled: using local provider for safer results")
        from ai.local_provider import LocalProvider
...
def show_provider_status(logger):
    ...
    logger.info("")
    logger.info("To use auto-generation (no API needed): use --ai local or set AI_PROVIDER=local")
