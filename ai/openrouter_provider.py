...
class OpenRouterProvider:
    ...
    def get_transcriber(self):
        """Return transcription function using local Whisper (OpenRouter doesn't provide audio APIs)"""
        import sys
    ...
    def filter_moments(self, candidates: List[Dict], transcript: List[Dict]) -> List[Dict]:
        """Filter moments using OpenRouter"""
        if len(candidates) == 0:
            return []

        print(f"  Filtering with OpenRouter ({self.CHAT_MODEL})...")
...
Answer only YES or NO:"""
        ...
        try:
            ...
            print(f"  Warning: OpenRouter check failed: {e}, using fallback")
            return True

    def score_moments(self, moments: List[Dict], transcript: List[Dict]) -> List[Dict]:
        """Score moments using OpenRouter"""
        print(f"  Scoring with OpenRouter ({self.CHAT_MODEL})...")
...
Reply with ONLY a number 0-100:"""
        ...
        try:
            ...
            print(f"  Warning: OpenRouter scoring failed: {e}, using fallback")
